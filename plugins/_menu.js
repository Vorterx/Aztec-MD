...
const { commands } = require('../lib/_cmd_sxntax');
...
Zenith(
    ...
    async (vorterx, m, react, { args }) => {
        ...
        await react('Ⓜ️');
        const cmd = commands.find((cmd) => cmd.usage === (args.split(" ")[0].toLowerCase()))
        if (cmd && cmd.dontAddCommandList === false && cmd.usage !== undefined) {
            if (!cmds[cmd.category]) cmds[cmd.category] = []
            cmds[cmd.category].push(cmd.usage)
        }
