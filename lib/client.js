...
module.exports = MessageHandler = async (messages, vorterx) => {
  try {
   ...
   // const args = body.replace(getCMD, '').slice(1).trim();
    const args = body.replace(getCMD, '').trim().split(/\s+/);
  ...
  vorterx.sendMessage(m.from,{
    ...
    if (command && typeof command.client === 'function') {
      command.client(vorterx, m, {
        name: "vorterx",
        pushName: m.pushName,args,isAdmin,isMedia,getCMD,isBotAdmin,mentionByTag,isDev,participants,mime,connect,quoted,
      });
    }
  } catch (err) {
    console.log(err, 'red');
