...
module.exports = MessageHandler = async (messages, vorterx) => {
  try {
    ...
    const args = inputString.trim().split(',').map(item => item.trim());
    const text = args.join("-");
    const isCmd = body.startsWith(prefix);
    const getCMD = body.slice(1).trim().split(/ +/).shift().toLowerCase();
    const arg = body.replace(getCMD, '').slice(1).trim();
  ...
  vorterx.sendMessage(m.from,{
    ...
    if (command && typeof command.client === 'function') {
      command.client(vorterx, m, {
        name: "vorterx",
        pushName: m.pushName,arg,args,isAdmin,isMedia,getCMD,isBotAdmin,mentionByTag,isDev,participants,mime,connect,quoted,
      });
    }
  } catch (err) {
    console.log(err, 'red');
