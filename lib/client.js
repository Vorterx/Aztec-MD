...
async function getJson(url, options) {
  try {
   ...
   m.reply('Error occurred while fetching JSON data.');
    ...
    command.client(vorterx, m, { name: "vorterx", pushName: m.pushName, arg, args, isAdmin, getJson, isBotAdmin, mentionByTag, participants, text, mime, connect, quoted, mode: mode.get(),});
  }
  } catch (err) {
  console.log(err, 'red')
  }
