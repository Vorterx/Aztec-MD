...
if (antilink && isGroup && groupAdmins.includes(vorterx.user.id.split(':')[0] + '@s.whatsapp.net') && body) {
  ...
  if (body === prefix) {
    ...
    console.log(
      ...
      (isGroup || isDev) || (await react("⚠️"), m.reply("*Inbox commands not allowed*"));
  ...
  const getErr = {
        contextInfo: {
           ...
           externalAdReply: {
              ...
              title: 'E3RR REPLY',
               body: `${m.pushName}`,
               thumbnail: await getBuffer("https://i.ibb.co/frX9YvD/OIG.jpg"), 
               sourceUrl: `${config.CHANNEL}`, 
          },
        },
        text: teks,
      };
      return vorterx.sendMessage(m.chat, getErr, {
        quoted: m,
      });
    }
