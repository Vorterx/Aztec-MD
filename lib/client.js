  ...
  try {
    ...
    async function doQuiz(getQuiz) {
      ...
      try {
        ...
        console.error(error);
        m.reply('__An error occurred during the quiz...');
      }
    }
    ...
    if (command && typeof command.client === 'function') {
      command.client(vorterx, m, {
        name: "vorterx",
        pushName: m.pushName,arg,args,isAdmin,isMedia,doQuiz,isBotAdmin,mentionByTag,participants,text,mime,connect,quoted,
        mode: mode.get()
      });
    }
  } catch (err) {
