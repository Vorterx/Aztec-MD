...
let Zenth = require("./_cmd_sxntax.js")
async function Aztec(messages, vorterx) {
  try {
    ...
    const cmd =
    Zenth.commands &&
  (Zenth.commands.find((cmd) => cmd.usage === getCMD) ||
  Zenth.commands.find((cmd) => cmd.alias && cmd.alias.includes(getCMD))) /*  ||
  Zenth.cmd.get(getCMD) || Zenth.cmd.find((cmd) => cmd.alias && cmd.alias.includes(getCMD)   );
         */
...
*/
    ...
    if (cmd && cmd.function && typeof cmd.function === 'function') {
      try {
        cmd.function(vorterx, coax,react, {
