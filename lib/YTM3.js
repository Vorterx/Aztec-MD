...
class YTM3 {
  ...
  static searchTrack = async (query) => {
    try {
      ...
      const result = ytMusic.map((music) => ({
        isYtMusic: true,
        ...
        album: music.album,
        duration: {
          seconds: music.duration.totalSeconds,
  ...
  static downloadMusic = async (query) => {
    try {
      ...
      const stream = ytdl(search.id, { filter: 'audioonly', quality: 140 });
      const songPath = `./download/audio/${randomBytes(3).toString('hex')}.mp3`;
  ...
  static mp3 = async (url, metadata = {}, autoWriteTags = false) => {
    try {
      ...
      const songPath = `./downloads/audio/${randomBytes(3).toString('hex')}.mp3`;
