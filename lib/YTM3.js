...
class YTM3 {
  ...
  static downloadMusic = async (query) => {
    try {
      const downloadDir = path.join(__dirname, 'lib', 'downloads', 'audio');
      ...
      let songPath = `${downloadDir}/${randomBytes(3).toString('hex')}.mp3`;
  ...
  static mp3 = async (url, metadata = {}, autoWriteTags = false) => {
    try {
      ...
      let songPath = path.join(__dirname, 'lib', 'downloads', 'audio', `${randomBytes(3).toString('hex')}.mp3`);
